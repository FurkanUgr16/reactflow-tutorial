...
export function BaseNode({ className, status, ...props }: BaseNodeProps) {
  return (
    ...
    >
      {props.children}
      {status === "error" && (
        <XCircleIcon className=" absolute right-0.5 bottom-0.5 size-2 text-red-400 stroke-3" />
      )}
      {status === "success" && (
        <CheckCircle2Icon className=" absolute right-0.5 bottom-0.5 size-2 text-emerald-600 stroke-3" />
      )}
